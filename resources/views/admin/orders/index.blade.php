<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if(session('success'))
                <div class="bg-green-100 dark:bg-green-900/50 border border-green-400 dark:border-green-600 text-green-700 dark:text-green-300 px-4 py-3 rounded mb-4">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <form method="GET" action="{{ route('admin.orders.index') }}" class="flex items-end space-x-4">
                        ...
                        <a href="{{ route('admin.orders.index') }}" class="bg-gray-200 dark:bg-slate-600 text-gray-700 dark:text-gray-200 px-6 py-2 rounded-md hover:bg-gray-300 dark:hover:bg-slate-500">
                            Reset
                        </a>
                    </form>
                </div>
                            ...
                            @forelse($orders as $order)
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ $order->orderItems->count() }} item</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-semibold text-gray-900 dark:text-white">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</div>
                                        @if($order->payment)
                                            <div class="text-xs">
