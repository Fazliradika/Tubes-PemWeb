    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-blue-100 text-sm mt-1">Perbarui informasi produk kesehatan yang dipilih.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-xl sm:rounded-2xl border border-blue-50 dark:border-slate-700">
                <form action="{{ route('admin.products.update', $product) }}" method="POST" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="price" id="price" value="{{ old('price', round($product->price)) }}" required min="0"
                                        class="w-full rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 transition-all"
                                        placeholder="0">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <div class="mt-4 p-2 border border-slate-100 dark:border-slate-700 rounded-xl inline-block bg-slate-50 dark:bg-slate-900">
                                    <img src="{{ $product->image ?: 'https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=100&h=100&fit=crop' }}" 
                                         alt="Preview" class="w-20 h-20 object-cover rounded-lg">
                                    <span class="text-[10px] text-slate-500 block text-center mt-1 uppercase font-bold tracking-tighter">Preview Saat Ini</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="5" required
                                    class="w-full rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 transition-all"
                                    placeholder="Jelaskan detail... ">{{ old('description', $product->description) }}</textarea>
